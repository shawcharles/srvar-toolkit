...
def test_backtest_metrics_csv_includes_pinball_when_enabled(monkeypatch, tmp_path) -> None:
    ...
    cfg = {
        ...
        "evaluation": {
            ...
            "pinball": {"enabled": True, "quantiles": [0.5], "use_latent": False},
            "pit": {"enabled": False},
        },
    ...
    assert header == "variable,horizon,crps,pinball,rmse,mae,coverage_50"
    ...
    assert float(rows[0]["pinball"]) == 0.0
    ...
    assert float(rows[1]["pinball"]) == 0.5
