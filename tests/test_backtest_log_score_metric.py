...
def test_backtest_metrics_csv_includes_log_score_when_enabled(monkeypatch, tmp_path) -> None:
    ...
    def fake_forecast(
        ...
        *,
        ...
        base = np.array([-1.0, 1.0, -1.0, 1.0], dtype=float)
        sims = np.tile(base[: int(draws), None, None], (1, hmax, int(fit.dataset.N)))
    ...
    assert header == "variable,horizon,crps,log_score,rmse,mae,coverage_50"
