...
def main() -> None:
    ...
    # Generate a latent rate process, then apply an ELB floor to obtain the observed rate.
    # This avoids a degenerate "observed" forecast that is stuck at the bound.
    ...
    fc = forecast(fit_res, horizons=[1, 4, 8, 12], draws=2000, rng=np.random.default_rng(2024))
    ...
    fig, ax = plot_shadow_rate(fit_res, var="r", bands=(0.1, 0.9))
    fig.savefig(out_dir / "shadow_rate_r.png", dpi=200)

    fig, ax = plot_volatility(fit_res, var="r", bands=(0.1, 0.9))
    fig.savefig(out_dir / "volatility_r.png", dpi=200)

    fig, ax = plot_forecast_fanchart(fc, var="r", bands=(0.1, 0.9), use_latent=False)
    fig.savefig(out_dir / "forecast_fan_r_observed.png", dpi=200)

    fig, ax = plot_forecast_fanchart(fc, var="r", bands=(0.1, 0.9), use_latent=True)
    fig.savefig(out_dir / "forecast_fan_r_shadow.png", dpi=200)
