...
def main() -> None:
    ...
    sampler = SamplerConfig(draws=1200, burn_in=300, thin=1)

    rng = np.random.default_rng(2024)
    fit_res = fit(ds, model, prior, sampler, rng=rng)
    fc = forecast(fit_res, horizons=[1, 4, 8, 12], draws=800, rng=rng)

    print("fit complete (ELB+SV)")
    print("forecast mean shape:", fc.mean.shape)
    print("has latent draws:", fc.latent_draws is not None)
