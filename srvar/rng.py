...
def gamma_rate(
    *, shape: float | np.ndarray, rate: float | np.ndarray, rng: np.random.Generator
) -> np.ndarray:
...
def inverse_gaussian(
    *, mu: float | np.ndarray, lam: float | np.ndarray, rng: np.random.Generator
) -> np.ndarray:
    ...
    if np.any(~np.isfinite(lam_arr)):
        raise ValueError("lam must be finite")

    m = np.clip(m, 1e-6, 1e6)
    lam_arr = np.clip(lam_arr, 1e-6, 1e6)

    v = rng.standard_normal(size=np.broadcast(m, lam_arr).shape) ** 2

    m_b = np.broadcast_to(m, v.shape)
    l_b = np.broadcast_to(lam_arr, v.shape)

    y = (
        m_b
        + (m_b * m_b * v) / (2.0 * l_b)
        - (m_b / (2.0 * l_b)) * np.sqrt(4.0 * m_b * l_b * v + (m_b * m_b) * (v * v))
    )
